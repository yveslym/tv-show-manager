...
public class DOFavoriteButton: UIButton {
    ...
    private func createLayers(image: UIImage!) {
        ...
        let imageFrame = CGRect(origin: CGPoint(x: frame.size.width / 2 - frame.size.width / 4, y: frame.size.height / 2 - frame.size.height / 4), size: CGSize (width: frame.size.width / 2,height: frame.size.height / 2))
        let midX = imageFrame.midX
        let midY = imageFrame.midY
        let imgCenterPoint = CGPoint(x: midX, y: midY)
        
        let lineFrame = CGRect(origin: CGPoint(x: imageFrame.origin.x - imageFrame.width / 4,y: imageFrame.origin.y - imageFrame.height / 4 ), size: CGSize(width: imageFrame.width * 1.5, height: imageFrame.height * 1.5))
        ...
        for i in 0 ..< 5 {
            ...
            line.path = {
                ...
                //path.move( to: ((, ), transform: )
                let lineMidX = lineFrame.midX
                let lineMidY = lineFrame.midY
                path.move(to: CGPoint(x: lineMidX, y: lineMidY))
